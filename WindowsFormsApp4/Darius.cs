...
namespace WindowsFormsApp4
{
    public class Darius : Character
    {
        ...
        Bitmap char_action = new Bitmap("Dariussprite_action.png");
        Bitmap creep_death = new Bitmap("creep_death.png");
        Bitmap background_char = new Bitmap("background.jpg");
        Bitmap Darius_death_1 = new Bitmap("Darius_death_2.png");
        Bitmap Darius_death_2 = new Bitmap("Darius_death_3.png");

        private Bitmap sprite = new Bitmap("Dariussprite.png");
        ...
        private int curFrameColumn_action = 0;

        private static Darius instance;

        public static Darius Instance
        {
            get { if (instance == null) instance = new Darius(); return instance; }
            private set { instance = value; }
        }

        public Darius()
        {
            char_sprite = new Bitmap("Dariussprite.png");
            avatar = new Bitmap("Dariusavatar.png");
        }
        public override void Correct_action(PaintEventArgs e, ref int isCorrect, ref int Q_check, ref int char_action_goback, ref int location_char, ref int location_creep)
        {
            ...
            {
                if (location_char <= location_creep - 150)
                {
                    location_char += 20;
                    location_creep -= 30;
                    e.Graphics.DrawImage(background_char, new Rectangle(0, 0, 1000, 600), new Rectangle(0, 0, 1280, 720), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(sprite, location_char, 385, new Rectangle(index * 110, 0, 88, 111), GraphicsUnit.Pixel);
                ...
                else
                {
                    ...
                    //e.Graphics.DrawImage(background_char, 2, 1);
                    e.Graphics.DrawImage(background_char, new Rectangle(0, 0, 1000, 600), new Rectangle(0, 0, 1280, 720), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(char_action, location_char, 385, new Rectangle(curFrameColumn_action * 132, 0, 150, 110), GraphicsUnit.Pixel);
                    // e.Graphics.DrawImage(creep_death, location_creep + 60, 420, new Rectangle(0, 0, 88, 110), GraphicsUnit.Pixel);
                    // check xem char da ve vi tri ban dau chua
                    if (curFrameColumn_action == 3)
                    {
                        e.Graphics.DrawImage(creep_death, location_creep + 60, 420, new Rectangle(0, 0, 88, 110), GraphicsUnit.Pixel);
                        ...
                        curFrameColumn_action = 0;
                    }
                    else
                    {
                        e.Graphics.DrawImage(creep, location_creep, 420, new Rectangle(0, 0, 84, 110), GraphicsUnit.Pixel);

                        curFrameColumn_action++;
                    }
            ...
            else if (Q_check == 1 && isCorrect == 1)
            {
                if (location_char <= 100)
                {
                    // e.Graphics.DrawImage(background_char, 2, 1);
                    e.Graphics.DrawImage(background_char, new Rectangle(0, 0, 1000, 600), new Rectangle(0, 0, 1280, 720), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(sprite, location_char, 385, new Rectangle(0, 0, 92, 113), GraphicsUnit.Pixel);
                ...
                if (char_action_goback == 2)
                {
                    // e.Graphics.DrawImage(background_char, 2, 1);
                    e.Graphics.DrawImage(background_char, new Rectangle(0, 0, 1000, 600), new Rectangle(0, 0, 1280, 720), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(sprite, location_char, 385, new Rectangle(index * 110, 120, 88, 113), GraphicsUnit.Pixel);
                    location_char -= 20;
                    if (index >= 3)
                        index = 0;
                    else
        ...
        public override void Incorrect_action(PaintEventArgs e, ref int isCorrect, ref int Q_check, ref int char_action_goback, ref int location_char, ref int location_creep)
        {
            ...
            if (Q_check == 0 && isCorrect == 2)
            {
                ...
                {
                    e.Graphics.DrawImage(background_char, new Rectangle(0, 0, 1000, 600), new Rectangle(0, 0, 1280, 720), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(Darius_death_1, location_char, 380, new Rectangle(0, 0, 84, 111), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(creep, location_creep, 420, new Rectangle(0, 0, 84, 110), GraphicsUnit.Pixel);
                    char_action_goback = 2;
                    index = 0;
                    Q_check = 1;
                ...
                else if (location_firebullet - 30 <= location_char && incorrect_check == 3)
                {
                    ...
                    e.Graphics.DrawImage(background_char, new Rectangle(0, 0, 1000, 600), new Rectangle(0, 0, 1280, 720), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(Darius_death_2, location_char, 380, new Rectangle(curFrameColumn_deadth_3 * 220, 0, 200, 120), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(creep, location_creep, 420, new Rectangle(0, 0, 84, 110), GraphicsUnit.Pixel);
                    if (curFrameColumn_deadth_3 == 10)
                    {
                        // MessageBox.Show("May thua roi thang lon :)))");
                        isCorrect = 4; //  cho nó dừng draw 
                ...
                else
                {
                    ...
                    e.Graphics.DrawImage(background_char, new Rectangle(0, 0, 1000, 600), new Rectangle(0, 0, 1280, 720), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(sprite, location_char, 385, new Rectangle(index * 110, 0, 84, 111), GraphicsUnit.Pixel);
            ...
            else if (Q_check == 1 && isCorrect == 2)
            {
                ...
                if (char_action_goback == 2)
                {
                    //e.Graphics.DrawImage(background_char, 2, 1);
                    e.Graphics.DrawImage(background_char, new Rectangle(0, 0, 1000, 600), new Rectangle(0, 0, 1280, 720), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(sprite, location_char, 385, new Rectangle(index * 110, 120, 84, 110), GraphicsUnit.Pixel);
                    e.Graphics.DrawImage(creep, location_creep, 420, new Rectangle(0, 0, 84, 110), GraphicsUnit.Pixel);
                    if (index >= 3)
                        index = 0;
                    else
